...
1. When calling `.save`, **Hibernate** will execute following **SQL query** behind the scenes.
    - Notice that the `id=1` is **not null** anymore, but the **Message** object is not inserted into the database yet.
    - Where is this `id=1` comes from, if there is no record in db?
        - Behind the scenes **Hibernate** is using **JDBC Transactions** is used.
            - This means `id` is generated by **MySQL**, which are yet to committed to the database, because usage of **Transaction**.
2. This will be finished after calling `session.getTranaction().commit()`.            
...
3. Example of **@annotation** of **persistent entity** in the Java object `@Entity`.
4. When using **annotation** remember to use, the configuration to the `<mapping> class="entity.Message"/>`. 
...
- We will run the project **twice**.
    - Once for old `.xml` mapping.
    - Other one with the `@annotations`.

- Example of `.xml` mapping file.
...
1. **NOTICE** the same domain package.